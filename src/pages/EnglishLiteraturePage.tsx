...
import { englishLiteratureGroups } from "../data/englishLiterature";

export default function EnglishLiteraturePage() {
  const [activeTab, setActiveTab] = useState(0);
  const [mode, setMode] = useState<"study" | "challenge">("study");
  const activeGroup = englishLiteratureGroups[activeTab];
  ...
  return (
    <div className="study-page">
      ...
      <p className="page-intro">
        Study major English authors, novels, drama, poetry, and literary terms.
      </p>
      <p className="study-note">Tap a card to flip it.</p>

      <div className="continent-tabs">
        {englishLiteratureGroups.map((group, index) => (
          <button
            key={group.name}
      ...
      {mode === "study" ? (
        <div className="countries-grid fact-grid english-literature-grid">
          {activeGroup.cards.map((card) => (
            <Flashcard
              ...
              wrapperClassName="english-literature-card"
            />
          ))}
        </div>
      ) : (
